...
2. ✅ **更新 pnpm-lock.yaml** - 修改 `package.json` 版本後，**必須執行 `pnpm install --lockfile-only`** 更新 lockfile，否則 CI 會因 `--frozen-lockfile` 失敗
3. ✅ **編譯無錯誤** - 後端 `dotnet build` 和前端 `npm run build` 均成功
4. ✅ **單元測試無錯誤** - 所有後端單元測試通過
5. ✅ **前端 UI 測試無錯誤** - 前端元件測試通過
6. ✅ **瀏覽器 UI 測試無錯誤** - 使用 Browser Automation Tools 進行 E2E 測試
7. ✅ **Jest 測試無異步洩漏** - 測試完成後不得出現 `Jest did not exit one second after the test run has completed` 警告，若出現必須立即修復
