...
export class WebServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/api/mcp-servers', (req, res) => {
      try {
        const data: CreateMCPServerRequest = req.body;
    ...
    this.app.put('/api/mcp-servers/:id', async (req, res) => {
      try {
        ...
        const data: UpdateMCPServerRequest = req.body;
    ...
    this.app.post('/api/mcp/execute-tools', async (req, res) => {
      try {
        ...
        for (const tool of tools) {
          ...
          try {
            const result = await mcpClientManager.callTool(toolInfo.serverId, name, args || {});
            const entry: { name: string; success: boolean; result?: unknown; error?: string | undefined } = {
              name,
    ...
    this.app.post('/api/mcp-servers/connect-all', async (req, res) => {
      try {
        ...
        const results: Array<{ id: number; name: string; success: boolean; error?: string | undefined }> = [];
    ...
    this.app.get('/api/api-logs', (req, res) => {
      try {
        ...
        const filter = req.query['filter'] as string | undefined; // 'all', 'success', 'errors'
        const limit = req.query['limit'] ? parseInt(req.query['limit'] as string) : 100;
        const offset = req.query['offset'] ? parseInt(req.query['offset'] as string) : 0;

        const options: { endpoint?: string; successOnly?: boolean; errorsOnly?: boolean; limit?: number; offset?: number } = { limit, offset };
        if (endpoint) {
    ...
    this.app.get('/api/error-logs', (req, res) => {
      try {
        ...
        const options: { endpoint?: string; limit?: number; offset?: number } = { limit, offset };
        if (endpoint) {
          options.endpoint = endpoint;
        }
    ...
    this.app.delete('/api/api-logs/cleanup', (req, res) => {
      try {
        const daysToKeep = req.query['days'] ? parseInt(req.query['days'] as string) : 7;
   ...
   */
  async startWithMCPEndpoints(mcpServer: MCPServer, transportMode: 'sse' | 'streamable-http'): Promise<void> {
   ...
   */
  private setupMCPHTTPEndpoints(transportMode: 'sse' | 'streamable-http'): void {
