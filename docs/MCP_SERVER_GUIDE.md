...
從 v2.2.0 開始，AI 可以自動識別並呼叫 MCP 工具。當您的問題需要使用工具時，AI 會：

1. **自動識別** - 根據對話內容判斷是否需要工具
2. **多輪執行** - 最多執行 5 輪工具呼叫
3. **進度顯示** - 即時顯示執行進度與狀態
4. **智慧確認** - 第 5 輪會詢問是否繼續
...
| POST | `/api/mcp/execute-tool` | 執行單一工具 (自動找 Server) |
| POST | `/api/mcp/execute-tools` | 批次執行多個工具 |

### AI 回覆 API

| 方法 | 端點 | 說明 |
|------|------|------|
| POST | `/api/ai-reply` | 生成 AI 回覆 (支援 includeMCPTools) |
...
{
  "toolCalls": [
    { "name": "get_current_time", "arguments": { "timezone": "Asia/Taipei" } },
...
{
  "results": [
    { "name": "get_current_time", "success": true, "content": [...] },
    { "name": "list_directory", "success": true, "content": [...] }
  ]
}
```
...
```json
{
  ...
  "toolResults": "已執行 get_current_time: 15:30:00"
}
```
...
**A:** v2.2.0 起，AI 會自動識別並呼叫適當的工具。只需確保 MCP Server 已連接，在對話中描述您的需求，AI 會自動選擇並執行相關工具。執行過程會顯示進度指示器。最多執行 5 輪，第 5 輪會詢問是否繼續。
