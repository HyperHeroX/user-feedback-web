...
export function createDefaultConfig() {
  return {
    ...
    imageToTextPrompt: getEnvVar(
      "MCP_IMAGE_TO_TEXT_PROMPT",
      "请详细描述这张图片的内容，包括主要元素、颜色、布局、文字等信息。"
    ),
  };
}
/**
...
export function displayConfig(config) {
  ...
  console.log(
    `  Kill Port Process: ${
      config.killProcessOnPortConflict ? "enabled" : "disabled"
    }`
  );
  console.log(
    `  Use Fixed URL: ${config.useFixedUrl ? "enabled" : "disabled"}`
  );
  console.log(
    `  Cleanup Port On Start: ${
      config.cleanupPortOnStart ? "enabled" : "disabled"
    }`
  );
  console.log(
    `  Image To Text: ${config.enableImageToText ? "enabled" : "disabled"}`
  );
  console.log(
    `  Image To Text Prompt: ${
      config.imageToTextPrompt
        ? config.imageToTextPrompt.substring(0, 50) + "..."
        : "default"
    }`
  );
}
