...
// 健康检查命令
program.command("health").description("检查配置和系统状态").action(healthCheck);
// 配置显示命令
program
  .command("config")
  .description("显示当前配置")
    ...
    try {
      ...
      try {
        const response = await fetch(
          `http://localhost:${server.getStatus().webPort}/api/metrics`
        );
...
program
  ...
  .option(
    "-m, --message <message>",
    "测试工作汇报内容",
    "这是一个测试工作汇报，用于验证collect_feedback功能是否正常工作。"
  )
  ...
  .action(async (options) => {
    try {
      ...
      try {
        const response = await fetch(
          `http://localhost:${server.getStatus().webPort}/api/test-session`,
          {
            method: "POST",
            headers: {
