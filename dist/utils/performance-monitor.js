...
export class PerformanceMonitor {
  ...
  recordRequest(responseTime, success) {
    ...
    if (this.requestStats.responseTimes.length > 1000) {
      this.requestStats.responseTimes =
        this.requestStats.responseTimes.slice(-1000);
    }
  }
  /**
   * 记录WebSocket连接
  ...
  recordWebSocketDisconnection() {
    this.websocketStats.activeConnections = Math.max(
      0,
      this.websocketStats.activeConnections - 1
    );
  }
  /**
   * 记录WebSocket消息
  ...
  recordSessionCompleted() {
    this.sessionStats.activeSessions = Math.max(
      0,
...
`;
  ...
  checkPerformanceWarnings() {
    ...
    if (metrics.requestStats.averageResponseTime > 2000) {
      warnings.push(
        `平均响应时间过长: ${metrics.requestStats.averageResponseTime.toFixed(
          2
        )} ms`
      );
    }
    ...
    // 会话超时警告
    const timeoutRate =
      metrics.sessionStats.totalSessions > 0
        ? (metrics.sessionStats.timeoutSessions /
            metrics.sessionStats.totalSessions) *
          100
        : 0;
    if (timeoutRate > 20) {
