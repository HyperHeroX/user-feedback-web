...
export class ImageProcessor {
  ...
  async compressImage(base64Data, options = {}) {
    try {
      ...
      // 转换回Base64
      const compressedBase64 = `data:${mimeType};base64,${outputBuffer.toString(
        "base64"
      )}`;
      logger.debug(
        `图片压缩完成: ${buffer.length} -> ${outputBuffer.length} bytes`
      );
  ...
  async validateAndProcessImage(imageData) {
    try {
      ...
      if (info.width > this.maxWidth || info.height > this.maxHeight) {
        ...
        const compressedInfo = await this.getImageInfoFromBase64(
          compressedData
        );
        return {
          ...imageData,
  ...
  getImageStats(images) {
    ...
    stats.averageSize =
      stats.totalCount > 0 ? stats.totalSize / stats.totalCount : 0;
    return stats;
  }
}
