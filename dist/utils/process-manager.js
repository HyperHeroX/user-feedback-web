...
export class ProcessManager {
  ...
  isOwnProcess(processInfo) {
    const ownProcessNames = ["node", "user-web-feedback", "tsx"];
    const ownKeywords = [
      "user-web-feedback",
      "cli.js",
      "cli.ts",
    ...
    if (isOwn) {
      logger.debug(
        `识别为自己的进程: ${processInfo.name} (PID: ${processInfo.pid})`
      );
  ...
  isSafeToKill(processInfo) {
    ...
    const safePrefixes = ["node", "npm", "npx", "user-web-feedback", "tsx"];
    ...
    for (const dangerous of dangerousNames) {
      if (
        processName.includes(dangerous) ||
        processCommand.includes(dangerous)
      ) {
        return false;
      }
    }
  ...
  async forceReleasePort(port) {
    ...
    if (this.isOwnProcess(processInfo)) {
      logger.info(
        `发现自己的僵尸进程，直接清理: ${processInfo.name} (PID: ${processInfo.pid})`
      );
    ...
    if (!this.isSafeToKill(processInfo)) {
      logger.warn(
        `不安全的进程，跳过终止: ${processInfo.name} (PID: ${processInfo.pid})`
      );
      return false;
    }
    // 尝试渐进式终止
    logger.info(
      `尝试终止安全进程: ${processInfo.name} (PID: ${processInfo.pid})`
    );
    const success = await this.progressiveKill(processInfo.pid);
    if (success) {
      // 等待端口释放
