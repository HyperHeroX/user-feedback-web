 ...
 * 取得定位命令的系統命令
 */
function getLocateCommand(toolName) {
 ...
 * 檢查單一 CLI 工具是否已安裝
 */
export async function checkToolInstalled(toolName) {
    const result = {
        name: toolName,
        installed: false,
        version: null,
 ...
 * 取得 CLI 工具版本
 */
export async function getToolVersion(toolName) {
 ...
 * 檢測所有支援的 CLI 工具
 */
export async function detectCLITools(forceRefresh = false) {
    // 檢查快取
    const now = Date.now();
    ...
    const toolNames = Object.keys(CLI_TOOLS_CONFIG);
