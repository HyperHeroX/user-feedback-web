...
class MCPClientManager extends EventEmitter {
    ...
    // ========== 延遲啟動相關方法 ==========
    deferredServersStarted = false;
    /**
     * 啟動所有已配置為延遲啟動的 MCP Servers
     * @param context 包含專案名稱和路徑的上下文
     */
    async startDeferredServers(context) {
        ...
        const results = await Promise.allSettled(deferredServers.map(config => {
     ...
     * @param context 包含專案名稱和路徑的上下文
     */
    resolveArgsTemplate(config, context) {
        const templateArgs = config.startupArgsTemplate || config.args || [];
        const resolvedArgs = templateArgs.map(arg => arg.replace(/\{project_path\}/g, context.projectPath)
