    ...
    if (data.shouldCloseAfterSubmit) {
      ...
      const countdownInterval = setInterval(() => {
        ...
        if (countdown > 0) {
          showStatusMessage(
            "success",
            `✅ 反馈提交成功！感谢您的宝贵意见。页面将在${countdown}秒后自动关闭。`
          );
        } else {
      ...
      showStatusMessage(
        "success",
        "✅ 反馈提交成功！感谢您的宝贵意见。页面将保持打开状态，您可以继续使用。"
      );
    ...
    if (data.session_id) {
      ...
      if (data.work_summary) {
        ...
        const hasExistingContent =
          lastWorkSummary && lastWorkSummary !== data.work_summary;
...
function showTab(tabName) {
  ...
  tabs.forEach((tab) => {
    if (
      (tabName === "feedback" && tab.textContent.includes("工作汇报")) ||
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    if (!currentFeedbackSession) {
      showStatusMessage(
        "error",
        "当前为演示模式，请通过MCP工具函数调用来创建正式的反馈会话"
      );
...
function submitFeedback(feedbackText, shouldCloseAfterSubmit) {
  ...
  const feedbackData = {
    ...
    imageDescriptions:
      imageDescriptions.length > 0 ? imageDescriptions : undefined, // 包含图片描述
    timestamp: Date.now(),
...
async function callChatAPI(messageText, images) {
  ...
  const response = await fetch(
    `${chatConfig.api_base_url}/v1/chat/completions`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
function showTimeoutWarning() {
  ...
  const remainingTime = Math.max(
    0,
    Math.ceil((sessionTimeoutDuration - (Date.now() - sessionStartTime)) / 1000)
  );

  showStatusMessage(
    "warning",
    `⏰ 会话即将在 ${remainingTime} 秒后超时，如未提交反馈将自动提交忙碌回复`,
    false
  );
...
function handleFrontendTimeout() {
  ...
  const countdownInterval = setInterval(() => {
    ...
    if (countdown > 0) {
      showStatusMessage(
        "info",
        `⏰ 会话超时，页面将在${countdown}秒后自动关闭。后端将自动提交忙碌回复。`,
        false
      );
    } else {
      clearInterval(countdownInterval);
