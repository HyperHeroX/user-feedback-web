...
export class WebServer {
  ...
  setupRoutes() {
    ...
    this.app.post("/api/convert-images", async (req, res) => {
      try {
        ...
        // 批量转换图片
        const descriptions =
          await this.imageToTextService.convertMultipleImages(images);
    ...
    this.app.get("/api/preferences", (req, res) => {
      try {
        ...
        res.status(500).json({
          success: false,
          error:
            error instanceof Error ? error.message : "獲取使用者偏好設定失敗",
        });
      }
    });
    // 更新使用者偏好設定
    this.app.put("/api/preferences", (req, res) => {
      try {
        ...
        res.status(500).json({
          success: false,
          error:
            error instanceof Error ? error.message : "更新使用者偏好設定失敗",
        });
      }
    });
    // 错误处理中间件
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on("request_session", () => {
        ...
        for (const [sessionId, session] of activeSessions) {
          if (
            !latestSession ||
            session.startTime > latestSession.session.startTime
          ) {
        ...
        if (latestSession) {
          // 有活跃会话，分配给客户端
          logger.info(
            `为客户端 ${socket.id} 分配会话: ${latestSession.sessionId}`
          );
      ...
      socket.on("user_activity", (data) => {
        logger.socket("user_activity", socket.id, {
          sessionId: data.sessionId,
        });
      ...
      socket.on("cancel_auto_reply", (data) => {
        logger.socket("cancel_auto_reply", socket.id, {
          sessionId: data.sessionId,
        });
        this.clearAutoReplyTimers(data.sessionId);
      ...
      socket.on("session_ready", (data) => {
        logger.socket("session_ready", socket.id, {
          sessionId: data.sessionId,
        });
  ...
  async handleFeedbackSubmission(socket, feedbackData) {
    ...
    try {
      ...
      if (
        !feedbackData.text &&
        (!feedbackData.images || feedbackData.images.length === 0)
      ) {
        socket.emit("feedback_error", {
          error: "请提供文字反馈或上传图片",
        });
      ...
      if (feedbackData.images && feedbackData.images.length > 0) {
        ...
        try {
          ...
          logger.info(
            `图片处理完成: ${stats.totalCount} 张图片, 总大小: ${(
              stats.totalSize /
              1024 /
              1024
            ).toFixed(2)}MB`
  ...
  startAutoReplyTimer(socket, sessionId, workSummary) {
    ...
    logger.info(
      `啟動自動回覆計時器: 會話 ${sessionId}, 超時 ${preferences.autoReplyTimeout} 秒`
    );
    // 設置警告計時器
    const warningTimer = setTimeout(() => {
      const remainingSeconds = Math.round((timeoutMs - warningMs) / 1000);
      logger.info(
        `發送自動回覆警告: 會話 ${sessionId}, 剩餘 ${remainingSeconds} 秒`
      );
    ...
    const autoReplyTimer = setTimeout(async () => {
      ...
      try {
        ...
        if (result.success && result.reply) {
          ...
          socket.emit("auto_reply_error", {
            error: result.error || "自動回覆生成失敗",
          });
        }
      } catch (error) {
  ...
  async start() {
    ...
    try {
      ...
      if (this.config.forcePort) {
        ...
        this.port = await this.portManager.resolvePortConflict(
          this.config.webPort
        );
      }
      // 启动服务器前再次确认端口可用
