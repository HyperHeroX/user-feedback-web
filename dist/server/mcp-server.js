...
export class MCPServer {
  ...
  registerTools() {
    ...
    this.mcpServer.registerTool(
      ...
      {
        description:
          "Collect feedback from users about AI work summary. This tool opens a web interface for users to provide feedback on the AI's work.",
        inputSchema: {
          work_summary: z
            .string()
            .describe("AI工作汇报内容，描述AI完成的工作和结果"),
        },
      },
      async (args) => {
        const params = {
  ...
  async collectFeedback(params) {
    ...
    logger.info(
      `开始收集反馈，工作汇报长度: ${work_summary.length}字符，超时: ${timeout_seconds}秒`
    );
    // 发送MCP工具调用开始通知
    ...
    feedback.forEach((item, index) => {
      ...
      if (item.images && item.images.length > 0) {
        ...
        item.images.forEach((img, imgIndex) => {
          ...
          content.push({
            type: "text",
            text: `图片 ${imgIndex + 1}: ${img.name} (${img.type}, ${(
              img.size / 1024
            ).toFixed(1)}KB)`,
          });
          // 添加图片描述（如果有）
          ...
          if (img.data) {
            ...
            const base64Data = img.data.replace(
              /^data:image\/[^;]+;base64,/,
              ""
            );
            content.push({
  ...
  formatFeedbackAsText(feedback) {
    ...
    feedback.forEach((item, index) => {
      ...
      if (item.images && item.images.length > 0) {
        ...
        item.images.forEach((img, imgIndex) => {
          parts.push(
            `  图片 ${imgIndex + 1}: ${img.name} (${img.type}, ${(
              img.size / 1024
            ).toFixed(1)}KB)`
          );
        });
  ...
  getStatus() {
    return {
      ...
      webPort: this.webServer.isRunning()
